...
from tkinter import messagebox  # Импорт из библиотеки Tkinter модуль messagebox (Всплывающие сообщения)
...
def calculate():
    ...
    try:
        ...
        messagebox.showinfo("Вы ввели неверный символ!", "Необходимо ввести цифры")
        calc.insert(0, 0)
...
def press_key(event):
    if event.char.isdigit():  # Проверяем ввод на цифры
        add_digit(event.char)  # Если цифры, то добавляем их функцией add_digit
    elif event.char in "+-/*.":  # Проверяем ввод на операции
        add_operation(event.char)  # Если операции, то добавляем из функцией add_operation
    elif event.char == "\r":  # Проверяем ввод клавиши Enter
        calculate()  # Если Enter, то добавляем из функцией calculate
