...
win.geometry("240x500+800+200")  # Задём размеры окна и размещение на экране
...
def add_operation(operation):
    value = calc.get()  # Переменная принимающая данные из окна ввода
    if value[-1] in "-+/*M":  # Проверка ввода данных
...
"""Функция создания кнопок ввода памяти"""


def make_memory_button(operation):
    return tk.Button(text=operation, bd=5, font=("Arial", 13), fg="red", command=mem_plus())

def make_memory_1_button(operation):
    return tk.Button(text=operation, bd=5, font=("Arial", 13), fg="red", command=mem_enter())


"""Создаём и оформляем кнопки операций"""

make_memory_button("M").grid(row=5, column=0, stick="wens", padx=5, pady=5)
make_memory_1_button("M-").grid(row=5, column=1, stick="wens", padx=5, pady=5)
